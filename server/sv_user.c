...
plainStrings:
	...
	else
	{
		...
		while (start < MAX_CONFIGSTRINGS)
		{
			...
			result = deflateInit2(&z, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 9, Z_DEFAULT_STRATEGY);
			if (result != Z_OK)
			{
				Com_Error(ERR_DROP, "ZLib data error! Error %d on deflateInit2.\nMessage: %s", result, z.msg);
...
plainLines:
	...
	else
	{
		...
		while (start < MAX_EDICTS)
		{
			...
			{
				SV_ClientPrintf (sv_client, PRINT_HIGH, "deflateInit2() failed. dohoh\n");
				goto plainLines;
			}
...
//	sizebuf_t	*queue;
	...
	if (sv_client->downloadCompressed)
	{
		...
		if (deflateInit2 (&z, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 9, Z_DEFAULT_STRATEGY) != Z_OK)
		{
			SV_ClientPrintf (sv_client, PRINT_HIGH, "deflateInit2() failed. ffs\n");
			SV_DropClient (sv_client, true);
			return;
		}
