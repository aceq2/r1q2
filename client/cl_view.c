...
void Get_Mset_fog(void) {
	int fog, r, g, b, a;
	int density, start, end;

	if (sscanf(cl.configstrings[CS_FOG], "%i %i %i %i %i %i %i %i",
		&fog, &r, &g, &b, &a, &density, &start, &end) != 8) {
		return;
	}

	Cvar_SetValue("gl_fog", (float)fog);
	Cvar_SetValue("gl_fog_r", (float)r/255);
	Cvar_SetValue("gl_fog_g", (float)g/255);
	Cvar_SetValue("gl_fog_b", (float)b/255);
	Cvar_SetValue("gl_fog_a", (float)a/255);
	Cvar_SetValue("gl_fog_density", (float)density*0.001f);
	Cvar_SetValue("gl_fogstart", (float)start);
	Cvar_SetValue("gl_fogend", (float)end);
}
/*
=================
CL_PrepRefresh
...
void Set_Fog(void) {
	...
	case 2:
		...
		glFogf(GL_FOG_DENSITY, gl_fog_density->value * 0.001f);
		break;
	case 3:
		...
		break;
	default:
		Com_Printf("ERROR: Invalid gl_fog value. GL_FOG set to 1.\n", LOG_CLIENT);
		Cvar_SetValue("gl_fog", 1);
	}
	GLfloat fogColor[] = { gl_fog_r->value,gl_fog_g->value,gl_fog_b->value,gl_fog_a->value };
...
void draw_rect(void) {
	...
	glBegin(GL_POLYGON);
		glVertex3f(1193,2827,292); // vertex 1
		glVertex3f(1193,1284,292); // vertex 2
		glVertex3f(-29,1284,292); // vertex 3
		glVertex3f(-29,2827,292); // vertex 4
	glEnd();
}

static void DrawStrafeHelper(void) {
	params.center = cl_strafeHelperCenter->intvalue;
	params.center_marker = cl_strafeHelperCenterMarker->intvalue;
	params.scale = (float)cl_strafeHelperScale->intvalue;
	params.height = (float)cl_strafeHelperHeight->intvalue;
	params.y = (float)cl_strafeHelperY->intvalue;
	params.colorcenter = cl_strafeColorCenter->intvalue;
	params.coloraccel = cl_strafeColorAccel->intvalue;
	params.coloroptimal = cl_strafeColorOptimal->intvalue;

	StrafeHelper_Draw(params, (float)scr_vrect.width, (float)scr_vrect.height);
}
/*
==================
V_RenderView
...
#endif
{
	...
	SCR_DrawCrosshair ();
	if (cl_drawStrafeHelper->intvalue) {
		DrawStrafeHelper();
	}
}
