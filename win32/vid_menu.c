...
static void DriverCallback2( void *unused )
{
...
//		"[Rendition OpenGL]",
	...
	else
	{
		...
		else if ( strcmp( gl_driver->string, "opengl32" ) == 0 )
		{
			...
			else if (strcmp(vid_ref->string, "pbgl") == 0)
				s_ref_list[s_current_menu_index].curvalue = REF_PBGL;
			else
	...
	for ( i = 0; i < 2; i++ )
	{
		...
		s_ref_list[i].generic.callback = DriverCallback2;
